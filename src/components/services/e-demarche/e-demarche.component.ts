...
import { EDemarcheService } from "../../../services/e-demarche.service";
...
export class EDemarcheComponent implements OnInit {
  ...
  private readonly eDemarcheService = inject(EDemarcheService);
  ...
  private readonly uploadedFileObjects: Record<string, File> = {};
  ...
  protected onFileSelected(doc: string, event: Event) {
    ...
    this.uploadedFileObjects[doc] = file;
  }
  ...
  protected removeFile(doc: string) {
    ...
    delete this.uploadedFileObjects[doc];
  }
  ...
  protected submit() {
    ...
    const personalKeys = PERSONAL_FIELDS.map((f) => f.key);
    const specificKeys = proc?.specificFields.map((f) => f.key) ?? [];

    const champsSpecifiques: Record<string, string> = {};
    for (const key of specificKeys) {
      champsSpecifiques[key] = raw[key] ?? '';
    }

    const documentLabels = proc?.docs ?? [];
    const files = documentLabels.map((label) => this.uploadedFileObjects[label]).filter(Boolean) as File[];

    const payload: EDemarcheRequestDTO = {
      type:             proc?.id ?? '',
      nom:              raw['nom'],
      prenom:           raw['prenom'],
      email:            raw['email'],
    ...
    this.eDemarcheService.submit(payload, files).subscribe({
      next: () => {
